...
header_target = "mx-auto relative"
header_replacement = "mx-auto"

services_pattern = re.compile(
    r'<!-- Services Dropdown -->\s*<div class="(relative\s+)?group\s+z-50">.*?Executive Search</a>\s*</div>\s*</div>',
...
industries_pattern = re.compile(
    r'<!-- Industries Dropdown -->\s*<div class="(relative\s+)?group">.*?Hospitality & Retail</a>\s*</div>\s*</div>',
    re.IGNORECASE | re.DOTALL
)
...
print(f"Dropdown positioning and sizing refactoring complete. {files_updated} files updated.")
