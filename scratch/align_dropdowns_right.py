...
services_pattern = re.compile(
    r'<!-- Services Dropdown -->\s*<div class="group z-50">.*?Executive Search</a>\s*</div>\s*</div>',
...
industries_pattern = re.compile(
    r'<!-- Industries Dropdown -->\s*<div class="group">.*?Hospitality & Retail</a>\s*</div>\s*</div>',
    re.IGNORECASE | re.DOTALL
)
...
for file_name in os.listdir(html_dir):
    if file_name.endswith(".html"):
        ...
        new_content = services_pattern.sub(services_replacement, content)
